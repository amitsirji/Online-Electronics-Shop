...
<?php
	...
	if(isset ($_POST['id']))
	{
		$id=$_POST['id'];
		$name=$_POST['name'];
		$path="product_img/lg_top_load_wm/".$id.".jpg";
		
		$capacity =$_POST['capacity'];
		$color =$_POST['color'];
		$pulsator =$_POST['pulsator'];
		$mrp =$_POST['mrp'];
		$new_mrp =$_POST['new_mrp'];
		$main_mrp =$_POST['main_mrp'];
		
		mysql_query("insert into lg_top_load_wm values('$id','$name','$path','$capacity','$color','$pulsator','$mrp','$new_mrp','$main_mrp')");
		
		if(mysql_affected_rows()>0)
		{
			copy($_FILES['file']['tmp_name'],"../product_img/lg_top_load_wm//".$id.".jpg");
		}
		else
		{
			echo"<script>
					alert('ID is Duplicat ! Record not Insert');
					window.location='lg_top_load_wm_add.php';</script>";
		}
	}	

?>
<h1><u>LG Top Load Washing Machine<br></u></h1>
		...
		<table border="0" style="float:left; margin-left:100px; border:double;">
			<th style="border-bottom:double;">Inserted ID List <br><br></th>
				...
				<?php
					$id_arr=mysql_query("select id from lg_top_load_wm");
		...
		<form action="lg_top_load_wm_add.php" method="post" enctype="multipart/form-data" onSubmit="return validate()">
			<table border="0" style="margin-right:200px">
				...
				<tr>
					<td>Color</td>
					<td><input type="text" class="text_box" id="id5" name="color" maxlength="40"></td>
					<td id="error_5" class="error">*</td>
				</tr>
				<tr>
					<td>Pulsator</td>
					<td><input type="text" class="text_box" id="id6" name="pulsator" maxlength="20"></td>
